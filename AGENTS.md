...
- In this repo, when building Storybook fixtures around local calendar dates or day selection, do not derive `occurred_at` with `toISOString().slice(0, 10)`. Use a local-date formatter such as `formatISO(date, { representation: "date" })` so stories do not drift across UTC boundaries and fail day-based assertions.

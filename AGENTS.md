...
- For authenticated local UI verification in this repo, prefer `npm run test:e2e:auth` with `MONIQ_E2E_EMAIL`/`MONIQ_E2E_PASSWORD` pointed at a seeded development user. A DevTools check against an already-authenticated browser tab is useful, but it is not a reproducible regression gate.

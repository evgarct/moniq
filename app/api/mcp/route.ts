              ...
              transactions: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    status: { type: "string", enum: ["paid", "planned"], description: "Use paid for settled transactions and planned for upcoming ones." },
            ...
            properties: {
              transactions: {
                ...
                items: {
                  ...
                  properties: {
                    title: { type: "string", description: "Merchant name or transaction description." },
                    amount: { type: "number", description: "Absolute transaction amount, always positive." },
                    occurred_at: { type: "string", description: "Transaction date in YYYY-MM-DD format." },
                    kind: { type: "string", enum: ["income", "expense"], description: "Whether this is income or an expense." },
                    suggested_category_name: {
                      type: "string",
                      description: "Best guess at a category name. The user can override it during review.",
                    },
                    currency: { type: "string", description: "3-letter ISO currency code. Optional." },
                    note: { type: "string", description: "Any extra context or original bank label. Optional." },
                  },
                },
                minItems: 1,
              },
              source_description: {
                type: "string",
                description: "Brief description of where these transactions came from.",
              },
            },
            required: ["transactions"],
          },
...
function buildCardAndDebtBalances(context: { wallets?: FinanceContextWallet[] }) {
...
async function handleGetCardAndDebtBalances(id: string | number | null, userId: string): Promise<McpResponse> {
  ...
  const balances = buildCardAndDebtBalances(data as { wallets?: FinanceContextWallet[] });
...
async function handleSubmitTransactionBatch(
  id: string | number | null,
  params: Record<string, unknown>,
  userId: string,
): Promise<McpResponse> {
...
async function handleToolCall(
  ...
  switch (toolName) {
    case "get_finance_context":
      return handleGetFinanceContext(id, userId);
    case "get_card_and_debt_balances":
      return handleGetCardAndDebtBalances(id, userId);
    case "create_transactions":
      return handleCreateTransactions(id, params, userId);
    case "submit_transaction_batch":
      return handleSubmitTransactionBatch(id, params, userId);
    default:

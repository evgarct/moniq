...
begin
  ...
  if p_statuses is not null and exists (
    ...
    where status not in ('paid', 'planned', 'skipped')
  ) then
  ...
  if p_kinds is not null and exists (
    select 1
    from unnest(p_kinds) kind
    where kind not in ('income', 'expense', 'transfer', 'debt_payment')
  ) then
  ...
  actual_transactions as (
    ...
    where t.user_id = key_user_id
      ...
      and (
        ...
        or t.source_account_id::text = any(p_account_ids)
        or t.destination_account_id::text = any(p_account_ids)
      )
  ...
  active_schedules as (
    ...
    left join public.wallets source_wallet
      on source_wallet.id = s.source_account_id
      and source_wallet.user_id = key_user_id
    left join public.wallets destination_wallet
      on destination_wallet.id = s.destination_account_id
      and destination_wallet.user_id = key_user_id
    left join public.finance_categories category
    ...
    where s.user_id = key_user_id
      ...
      and (
        p_account_ids is null
        or s.source_account_id::text = any(p_account_ids)
        or s.destination_account_id::text = any(p_account_ids)
      )
    ...
    cross join lateral (
      ...
      from (
        ...
        from generate_series(
          ...
          least(coalesce(s.until_date, p_end_date), p_end_date),
          interval '1 day'
        ) gs
        ...
        from generate_series(
          (
            s.start_date
            + (
              greatest(0, ceil(((p_start_date - s.start_date)::numeric) / 7.0)::integer) * 7
            )
          )::date,
          least(coalesce(s.until_date, p_end_date), p_end_date),
          interval '7 days'
        ...
        from generate_series(
          ...
          greatest(
            0,
            (
              (extract(year from least(coalesce(s.until_date, p_end_date), p_end_date))::integer * 12)
              + extract(month from least(coalesce(s.until_date, p_end_date), p_end_date))::integer
            )
            -
            (
              (extract(year from s.start_date)::integer * 12)
              + extract(month from s.start_date)::integer
            )
            + 1
          )
        ) month_index
        cross join lateral (
          select (date_trunc('month', s.start_date)::date + (month_index::text || ' months')::interval)::date as target_month
        ) target
        cross join lateral (
          select make_date(
            ...
            least(
              extract(day from s.start_date)::integer,
        ...
        where s.frequency = 'monthly'
          ...
          and monthly_occurrence.occurrence_date <= least(coalesce(s.until_date, p_end_date), p_end_date)
      ) occurrence_candidates
      where occurrence_date >= p_start_date
  ...
  select count(*)
  into returned_transaction_count
  ...
  transaction_payload as (
    select coalesce(
      jsonb_agg(payload order by occurred_at desc, created_at desc, sort_id desc),
      '[]'::jsonb
    ) as transactions
    from combined_transactions
  ),
  ...
  select jsonb_build_object(
    ...
    'limits', jsonb_build_object(
      ...
      'returned_transactions', returned_transaction_count
    )
  )
  || case
